...
/* when use mf_starpu broker, add this header file */
...
static unsigned nslicesx = 1;
static unsigned nslicesy = 1;
...
static void mult_kernel_common(void *descr[], int type)
{
	...
	if (type == STARPU_CPU)
	{
		...
		//printf("CPU result show: the 1th  subC is %f\t", subC[0]);
		//printf("CPU result show: the last subC is %f\t", subC[nxC*nyC-1]);
	}
#ifdef STARPU_USE_CUDA
	else
	{
		...
		TYPE *cpu_subC = (TYPE *)malloc(nxC*nyC*sizeof(TYPE));
		cudaMemcpy(cpu_subC, subC, nxC*nyC*sizeof(TYPE), cudaMemcpyDeviceToHost);
		//printf("GPU result show: the 1th  subC is %f\t", cpu_subC[0]);
		//printf("GPU result show: the last subC is %f\t", cpu_subC[nxC*nyC-1]);
	}
#endif
}
...
static struct starpu_perfmodel sgemm_perf_model =
{
	.type = STARPU_HISTORY_BASED,
	.symbol = "sgemm_history_perf"
};

/* when use mf_starpu broker, add this power model */
static struct starpu_perfmodel sgemm_power_model =
{
	.type = STARPU_HISTORY_BASED,
	.symbol = "sgemm_history_power"
};

/* when use mf_starpu broker, add codelet power model */
static struct starpu_codelet cl =
{
	...
	.model = &sgemm_perf_model,
	.power_model = &sgemm_power_model
};
